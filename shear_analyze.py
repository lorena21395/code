...
for fl in lists:
    ...
    e1_err = e1.std(axis=0)/np.sqrt(w.size)
    e2_err = e2.std(axis=0)/np.sqrt(w.size)
    ...
    """
    w, = np.where(data['nobj_found']!=1000)
    data = data[w]
    nobj_found = data['nobj_found']
    nobj = data['nobj']
    frac_found = np.sum(nobj_found)/np.sum(nobj)
    print("fraction of obj found: %g" % (frac_found))
    """
...
#plt.xlim(range_p1_min-0.002,range_p1_max+0.002)
plt.xlim(-0.4,0.4)
plt.title("Position Distributions (r = 7)")
plt.xlabel("position (pixel distance to center)")
...
plt.legend(loc = 0)
plt.title("Responses for 1 Galaxy")
plt.xlabel("R")
#plt.xlim(range_R11_min+0.02,range_R11_max+0.02)
plt.xlim(0.95,1.0)
plt.savefig('1-gal-responses.png')
plt.close()

binwidth = 0.01
bins1 = np.arange(min(e1), max(e1) + binwidth, binwidth)
bins2 = np.arange(min(e2), max(e2) + binwidth, binwidth)

range_e1_min = e1.mean()-5.*e1.std(axis=0)
range_e1_max = e1.mean()+5.*e1.std(axis=0)
range_e2_min = e2.mean()-5.*e2.std(axis=0)
range_e2_max = e2.mean()+5.*e2.std(axis=0)

plt.hist(e1,bins1,histtype='step',label=("e1 mean,std: "+ str(round(e1.mean(),3))+"+/-" +str(round(e1_err,3))+", "+str(round(e1.std(axis=0),3))),range=(range_e1_min,range_e1_max))
plt.hist(e2,bins2,histtype='step',label=("e2 mean,std: " + str(round(e2.mean(),3)) +"+/-" +str(round(e2_err,3))+", "+str(round(e2.std(axis=0),3))),range=(range_e2_min,range_e2_max))
plt.legend(loc = 0)
plt.title("Ellipticities for 1 Gal")
plt.xlabel("e")
plt.xlim(range_e1_min-0.002,range_e1_max+0.002)
#plt.yscale('log')
plt.savefig('1-gal-ellipticity.png')
plt.close()
