...
flist = glob('/gpfs01/astro/workarea/lmezini/scarlet-tests/run122/run*.fits')
...
plt.plot(corr_big_avg[24,:])
plt.ylim(-15,15)
plt.tight_layout()
plt.savefig('corr_cross_section.png')

"""
plt.imshow(corr_avg,interpolation='nearest', cmap='gray',vmin = np.min(corr_avg),vmax =np.max(15.))#corr_big_avg))#,norm=asinh)
plt.colorbar()
plt.title("Averaged Noise Correlation")
plt.savefig('test.png')
"""
